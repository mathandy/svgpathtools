...
class Arc(object):
    ...
    def _t_is_on_arc(self, t, point_on_line, arc_side):

        """This is a helper function for Arc.intersect(), to tell if a
        given value of `t` lies on the Arc segment or off it.

        It is needed because the list of parameterized intersection
        points generated by Arc.intersect() don't take into account the
        start and end point of the Arc, and because the parameter values
        are not restricted to the [0.0, 1.0] range.

        This function works by checking the sign of the dot product
        of two vectors.  One vector points from the "closing line"
        (the line connecting the start and end points of the Arc) to a
        point on the Arc, and the other vector points from the closing
        line to the (possible) intersection point.  Their dot product
        is positive if they point to the same side of the closing line
        (intersection point is on the Arc), and negative if they point
        to opposite sides (intersection point is off the Arc)."""

        p = self.point(t)
        p_side = complex(p.real - point_on_line.real, p.imag - point_on_line.imag)
        dot = (p_side.real * arc_side.real) + (p_side.imag * arc_side.imag)
        if dot < -0.000000001:
            return False
        return True
    ...
    def intersect(self, other_seg, tol=1e-12):
        ...
        if is_bezier_segment(other_seg):
            ...
            intersections = list(zip(t1s, t2s))
            if isinstance(other_seg, Line):
                # Find the line that joins the end points of the Arc.
                # Filter out all the intersections that line on the
                # opposite side of that line from the Arc.
                arc_closing_line = Line(self.start, self.end)
                point_on_arc = self.point(0.5)
                point_on_line = arc_closing_line.point(0.5)
                arc_side = complex(point_on_arc.real - point_on_line.real, point_on_arc.imag - point_on_line.imag)
                point_test = lambda t: self._t_is_on_arc(t[0], point_on_line, arc_side)
                intersections = filter(point_test, intersections)
